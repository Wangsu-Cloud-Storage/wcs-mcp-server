...
class _ToolImpl:
    ...
    @tools.tool_meta(
        types.Tool(
            name="GetObjectURL",
            description="Get the file download URL, and note that the Bucket where the file is located must be bound to a domain name. If using WOS's test domain, HTTPS access will not be available, and users need to make adjustments for this themselves.",
            inputSchema={
                "type": "object",
                "properties": {
                    "bucket": {
